/*
  ...
  + Wien Au�entemperatur + Luftfeuchtigkeit (Open-Meteo)
  + Sprachausgabe via Flite TTS ( Englisch )
  
  Audio: ES8311 Codec via I2S, Verst�rker GPIO42/46
  Taster: GPIO18 (BOOT) zum manuellen Ausl�sen der Sprachausgabe

  WLAN-SETUP: Bei erstmaligem Start oder nach �nderung der Zugangsdaten
  �ffnet sich automatisch ein Setup-Portal ("TempHumPow-Setup").
*/
...
DriverPins audioPins;                        // Pin-Zuordnung wird in initAudio() befüllt
AudioBoard audioBoard(es8311Driver, audioPins);
I2SCodecStream i2sOut(audioBoard);            // übernimmt I2C-Init des Codecs automatisch
Flite flite(i2sOut);
...
void initAudio() {
  ...
  digitalWrite(AUDIO_PWR_PIN, LOW);  // LOW = Verst�rker AN
  delay(200);

  // Pin-Zuordnung für den Codec: I2C (scl, sda, adresse) + I2S (mclk, bclk, ws, dout, din)
...
void speakWeather() {
  ...
  if (gWifiOk && !isnan(gOutTemp) && !isnan(gOutHum)) {
    // Wien-Daten verf�gbar: temperatur + feuchtigkeit
...
void setup() {
  ...
  if (gWifiOk) {
    ...
    fetchViennaWeather();
    Serial.printf("[WETT] Au�entemp: %.1f C, Feucht: %.1f%%, Regen: %d\n", gOutTemp, gOutHum, gRain);
  }
  ...
  Serial.println("[LOOP] Taster dr�cken f�r TTS...");
}

// --- Loop: Taster-Handling ---
void loop() {
  // BOOT-Taster (GPIO18) pr�fen: LOW = gedr�ckt
  if (digitalRead(PIN_PWR_BUTTON) == LOW) {
    delay(50);  // Entprellen
    if (digitalRead(PIN_PWR_BUTTON) == LOW) {
      Serial.println("[TTS] Taster gedr�ckt!");
      speakWeather();
